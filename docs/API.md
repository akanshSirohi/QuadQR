...
Starts live camera scanning. On browsers that expose camera controls, QuadQR requests continuous autofocus, exposure, and white balance. Modern browsers use two module Web Workers by default: a **small structural locator worker** (384 px by default) and a separate **full-recovery worker** for normal decoding plus the complete perspective, color, high-resolution, ECC, damaged-code, and multi-frame recovery stack. The locator never performs payload/color decoding on empty frames. The loop uses `requestVideoFrameCallback()` when available and applies latest-frame-wins scheduling, so only the newest available camera frame is captured after a busy locator finishes. Worker transport is runtime-probed: resized `ImageBitmap` is preferred, with automatic Canvas/transferable-`ImageData` fallback for Safari/WebKit failures. Repeated worker transport failures trigger a worker restart and finally a main-thread compatibility fallback rather than leaving the camera in a permanent error loop. Finder acquisition uses the streaming 1:1:3:1:1 RGB-value pass, direct cross-checks, local-threshold fallback, and directional module-size/version estimation. Once a geometry candidate validates structure, ECC, and CRC, it returns immediately. A miss with strong finder evidence dispatches parallel full recovery quickly. One finder must persist across consecutive fresh frames before recovery is armed, while zero-finder frames stay on the lightweight detector only. That recovery can use up to 960 px and retains the full QuadQR Auto Color crop sequence, center-weighted color recovery, threshold bracketing, precise alignment, perspective recovery, affine cross-channel calibration, QR-region enhancement, multi-frame confidence fusion, erasure decoding, and soft-decision Spectrum ECC.

For repeated scanning, use `continuous: true`. It is a public shorthand for keeping the camera active; the older `stopOnResult: false` form remains supported. Continuous mode creates a freshness boundary after every successful decode so a slow result from the other worker lane cannot be delivered afterward. The same decoded CRC is suppressed for `duplicateCooldown` milliseconds (1200 ms by default in continuous mode, `0` disables suppression). `pauseWhenHidden` defaults to `true`; the returned scanner also has `pause()` and `resume()` methods. Pass `signal: abortController.signal` for cancellation. If the camera track ends unexpectedly, QuadQR cleans up the scanner and reports the lifecycle through optional `onCameraState(event)`. The automatic main-thread fallback now gates expensive recovery on finder/geometry evidence instead of miss count alone.

```js
const scanner = await startCameraScanner(video, {
  cameraWorker: true,
  cameraLocatorMaxDimension: 384,
  maxDimension: 640,
  scanInterval: 33,
