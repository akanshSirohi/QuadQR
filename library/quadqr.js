...
function resolveLogoRenderOptions(options, matrixSize, moduleSize, quietZone, palette) {
  ...
  const rawSize = Number(nested?.size ?? options.logoSize ?? 0.18);
  const sizeRatio = Math.max(0.05, Math.min(0.30, Number.isFinite(rawSize) ? rawSize : 0.18));
...
export function renderToCanvas(codeOrMatrix, canvas, options = {}) {
  ...
  const size = matrix.length;
  const logo = resolveLogoRenderOptions(options, size, moduleSize, quietZone, palette);
...
export function renderToImageData(codeOrMatrix, options = {}) {
  ...
  const resolvedPalette = resolvePalette(options.palette);
  const logo = resolveLogoRenderOptions(options, size, moduleSize, quietZone, resolvedPalette);
