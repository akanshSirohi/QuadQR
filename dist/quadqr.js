...
function samplePerspectiveTriangleMatrix(imageData, homography, size, options = {}) {
...
function sampleAxisAlignedTriangleGrid(imageData, bounds, size, radiusRatio = 0.065, insetRatio = 0.27) {
  const moduleW = bounds.width / size;
  const moduleH = bounds.height / size;
...
return { computeHomography, projectPoint, detectCodeGeometry, autoColorImageData, autoToneContrastColorRgbGrid, autoToneContrastColorImageData, samplePerspectiveMatrix, samplePerspectiveTriangleMatrix, sampleObservedPalette, spatiallyNormalizeRgbGrid, rectifyImageData, findActiveBounds, sampleAxisAlignedGrid, sampleAxisAlignedTriangleGrid, visionInternals };
})();
...
const { detectCodeGeometry, computeHomography, projectPoint, samplePerspectiveMatrix, samplePerspectiveTriangleMatrix, rectifyImageData, sampleObservedPalette, spatiallyNormalizeRgbGrid, autoToneContrastColorRgbGrid, autoToneContrastColorImageData, autoColorImageData, findActiveBounds, sampleAxisAlignedGrid, sampleAxisAlignedTriangleGrid } = __qqr_vision_js;
...
const CELL_ENCODINGS = Object.freeze({
  RGBW: "rgbw",
...
const COMPRESSION_MODES = Object.freeze({
  NONE: "none",
  AUTO: "auto",
  LZ: "lz"
});
const SIGNATURE_ALGORITHMS = Object.freeze({ ED25519: "Ed25519" });
...
function decompressPayload(input, expectedLength = null) {
  const bytes = asBytes(input);
  const out = [];
  let pos = 0;
  while (pos < bytes.length) {
...
async function deriveSigningKeyId(publicKey, bytes = 8) {
  const subtle = subtleCrypto();
...
async function encodeSignedText(text, options = {}) {
...
function decodeUint8Array(inputMatrix, options = {}) {
...
async function verifyDecodedSignature(result, options = {}) {
...
function estimateSafeLogoSize(codeOrMatrix, options = {}) {
 ...
 */
function findMaxSafeLogoSize(code, options = {}) {
...
function getPrintGuidance(codeOrMatrix, options = {}) {
...
function renderToSVG(codeOrMatrix, options = {}) {
  const matrix = Array.isArray(codeOrMatrix) ? codeOrMatrix : codeOrMatrix.matrix;
  assert(Array.isArray(matrix) && matrix.length > 0, "A matrix is required.");
  const sizing = resolveRenderSizing(options, matrix.length);
  const { moduleSize, quietZone, pixelSize } = sizing;
...
function applyStressDistortion(imageData, type, severity = 0.5, options = {}) {
...
function runImageStressTest(imageData, expected = {}, options = {}) {
...
function runReliabilityLab(imageData, expected = {}, options = {}) {
...
function runPerspectiveSweep(imageData, expected = {}, options = {}) {
...
function assessScanability(code, renderOptions = {}, options = {}) {
...
function debugScanImageData(imageData, options = {}) {
  try {
...
return { compressPayload, decompressPayload, deriveSigningKeyId, generateSigningKeyPair, installCrc32Accelerator, crc32, getVersionInfo, encodeText, encodeBytes, encodeUint8Array, encodeSignedBytes, encodeSignedText, encodeSecureText, encodeSecureBytes, decryptDecoded, decodeMatrix, decodeUint8Array, verifyDecodedSignature, estimateSafeLogoSize, findMaxSafeLogoSize, getPrintGuidance, renderToCanvas, renderToImageData, renderToSVG, applyStressDistortion, runImageStressTest, runReliabilityLab, runPerspectiveSweep, assessScanability, debugScanImageData, scanImageData, scanFile, scanVideoFrame, startCameraScanner, rectifyDetectedCode, rotateMatrix, FORMAT_VERSION, MIN_VERSION, MAX_VERSION, DEFAULT_ECC_LEVEL, RENDER_MODES, CELL_ENCODINGS, PRINT_PALETTE, COMPRESSION_MODES, SIGNATURE_ALGORITHMS, CELL, DEFAULT_PALETTE, RENDER_STYLES, ECC_LEVELS, STRESS_PROFILES, RELIABILITY_PROFILES, internals, SECURITY_MODES, SECURITY_ALGORITHMS, SECURE_PAYLOAD_VERSION, DEFAULT_PBKDF2_ITERATIONS, generateRaw256Key, normalizeRaw256Key, bytesToHex };
})();
